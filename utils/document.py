...
class BaseDocument:
    ...
    def _row_speakers_default(self) -> list[str]:
        ...
        default_interviewer_name = self.default_speaker_pair[0]
        default_participant_name = self.default_speaker_pair[1]
...
class TextDocument(BaseDocument):
    """
    Wrapper class around BaseDocument that allows one to get information out of
    a document given a path to its location in the data directory.
    """
    ...
    def __repr__(self) -> str:
        return f"TextDocument({self.path})"

class DatasaurDocument(BaseDocument):
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        full_lines = [row['content'] for row in self.row_data]
        super().__init__('\n'.join(full_lines))
    ...
    def label_counts(self) -> dict[str, str]:
        ...
        for label, speaker in set(
            product(LABELS, DatasaurDocument.default_speaker_pair)
    ...
    def __repr__(self) -> str:
        return f"DatasaurDocument({self.name}, {self.project})"